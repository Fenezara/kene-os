...
import {
  ...
  Package, ScanFace, MessageSquare, Star, Gift, Wallet, Award, FileText, Lock
} from 'lucide-react'
...
function SidebarContent({ pathname, onNavigate }: { pathname: string; onNavigate?: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 pt-5 pb-3">
        ...
        <div className="mt-3 flex items-center gap-2">
          <button 
            ...
            className="flex-1 flex items-center gap-2 px-3 py-1.5 bg-white/5 hover:bg-white/10 border border-white/10 rounded-xl text-white/40 transition-colors"
          >
            <Search className="w-3.5 h-3.5 shrink-0 text-[#FFD700]" />
      ...
      {/* Footer */}
      <div className="p-3 border-t border-white/10 flex items-center justify-between text-xs text-white/40 bg-[#0F0A05]">
        <div className="flex items-center gap-1.5">
          <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
          <span className="text-[10px] font-mono text-white/60">v2.4 • PRO ({activePlan.toUpperCase()})</span>
        </div>
        <button
...
export default function ProLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className={`min-h-screen ${containerBg} flex flex-col md:flex-row font-sans w-full overflow-x-hidden transition-all duration-700`}>
      ...
      <aside className={`hidden lg:flex flex-col w-64 shrink-0 sticky top-0 h-screen z-20 transition-all duration-500 ${
      ...
      <div className="lg:hidden flex items-center justify-between px-4 h-14 bg-[#110D09]/95 backdrop-blur-xl border-b border-white/5 w-full shrink-0 z-30 sticky top-0">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <span className="text-[9px] text-white/40 block truncate">Plan : {activePlan.toUpperCase()}</span>
          </div>
        </div>
        ...
        <div className="hidden lg:flex items-center justify-between px-8 py-3.5 border-b border-white/10 bg-[#110D09]/90 backdrop-blur-xl sticky top-0 z-30">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-white/5 border border-white/10 text-white text-xs font-medium shadow-sm">
              ...
              <span className="text-white/60 text-[11px]">Session :</span>
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-[10px] text-white/50 font-mono uppercase tracking-wider bg-white/5 px-3 py-1.5 rounded-xl border border-white/10">
              <span className="text-[#FFD700] font-bold">Espace Salon Pro</span>
              <span>/</span>
