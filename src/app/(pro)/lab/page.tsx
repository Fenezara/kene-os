...
const INITIAL_LAB_ORDERS: LabFormulaOrder[] = [
  {
    ...
    formulaName: 'SÃ©rum Magistral Ã‰clat Bissap & Niacinamide Sur-Mesure',
    ...
    ingredients: [
      ...
      { name: 'Extrait ConcentrÃ© de Bissap (Hibiscus)', amount: '3.5 ml', category: 'AHA Naturel Ã‰clat' },
    ...
    bottleSize: 'Flacon Dropper Verre AmbrÃ© 30ml',
    batchNumber: 'LOT-2024-ABJ-042',
    expiryDate: '21 Octobre 2026 (3 Mois)',
    preparedBy: 'Fatou KonÃ© (Chimiste/Praticienne)',
  },
  {
    id: 'LAB-2024-002',
    clientName: 'Fatou Sarr',
    diagnosisId: 'DIAG-2024-092',
    formulaName: 'Baume RÃ©gÃ©nÃ©rant KaritÃ© Brut & ChÃ©bÃ© Cuir Chevelu',
    date: 'Aujourd\'hui 13:40',
    status: 'ready',
    phototype: 'Cheveux 4C CrÃ©pus',
    ingredients: [
      { name: 'Beurre de KaritÃ© Brut Bio Non RaffinÃ©', amount: '40 g', category: 'Beurre Nourrissant' },
      { name: 'Poudre de ChÃ©bÃ© Authentique', amount: '5 g', category: 'Fortifiant Capillaire' },
      { name: 'Macerat Huileux de Neem & Moringa', amount: '10 ml', category: 'Purifiant Cuir Chevelu' },
      { name: 'Huile Essentielle de Menthe PoivrÃ©e', amount: '3 gouttes', category: 'Stimulant Circulatoire' },
    ],
    bottleSize: 'Pot Verre AmbrÃ© 60ml',
  ...
  },
  {
    ...
    formulaName: 'Ã‰lixir Apaisant Anti-Harmattan Neem & Aloe',
    date: 'Hier 16:30',
    status: 'delivered',
    phototype: 'Phototype VI (Peau TrÃ¨s SÃ¨che)',
    ingredients: [
      ...
      { name: 'GlycÃ©rine VÃ©gÃ©tale', amount: '3 ml', category: 'Humectant' },
    ],
...
const RAW_MATERIALS = [
  { name: 'Beurre de KaritÃ© Brut Non RaffinÃ© ðŸ‡¬ðŸ‡­', stock: '12.5 kg', status: 'optimal', category: 'Beurres' },
  { name: 'Huile de Baobab PressÃ©e Ã  Froid ðŸ‡¸ðŸ‡³', stock: '4.8 Litres', status: 'optimal', category: 'Huiles' },
  { name: 'Extrait ConcentrÃ© de Bissap (Hibiscus) ðŸ‡¨ðŸ‡®', stock: '1.2 Litres', status: 'low', category: 'Actifs AHA' },
  { name: 'Poudre de ChÃ©bÃ© Authentique ðŸ‡¹ðŸ‡©', stock: '2.8 kg', status: 'optimal', category: 'Poudres' },
  ...
  { name: 'Flacons Verre AmbrÃ© 30ml Dropper', stock: '142 unitÃ©s', status: 'optimal', category: 'Conditionnement' },
];
...
export default function LabPage() {
  ...
  const handleUpdateStatus = (orderId: string, newStatus: LabFormulaOrder['status']) => {
    ...
    toast({
      title: 'ðŸ§ª Statut Laboratoire Mis Ã  Jour',
      description: `Ordre ${orderId} passÃ© en statut "${newStatus === 'ready' ? 'PrÃªt pour Flaconnage' : newStatus === 'delivered' ? 'LivrÃ© Ã  la Cliente' : 'En PrÃ©paration'}".`,
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="relative rounded-3xl p-6 overflow-hidden shadow-2xl border border-[#C8951E]/40 min-h-[160px] flex items-center">
        <img
          ...
          alt="Laboratoire CosmÃ©tique Africain"
          ...
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-[#C8951E] uppercase tracking-widest font-display flex items-center gap-1.5 bg-[#C8951E]/20 px-3 py-1 rounded-full border border-[#C8951E]/40 backdrop-blur-md">
                <Beaker className="w-4 h-4 text-[#C8951E]" /> Laboratoire de Confection CosmÃ©tique Sur-Mesure
              </span>
            </div>
            <h1 className="font-display font-black text-2xl text-white">
              Gestion de la PrÃ©paration Dermo-Botanique en Institut
            </h1>
            <p className="text-xs text-white/70 font-sans max-w-2xl">
              Formulation en direct, dosage au gramme/goutte, impression d'Ã©tiquettes personnalisÃ©es et suivi des matiÃ¨res premiÃ¨res brutes pour vos clientes.
            </p>
          </div>
                ...
                >
                  <div className="flex justify-between items-start">
                    ...
                    <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full border ${
                      ...
                      {order.status === 'in_preparation' ? 'ðŸ§ª En PrÃ©paration' : order.status === 'ready' ? 'âœ… PrÃªt pour Flacon' : 'ðŸ“¦ LivrÃ©'}
                    </span>
                  </div>
          ...
          <div className="lg:col-span-7">
            <div className="bg-[#1A1410] border border-white/10 rounded-3xl p-6 space-y-6 shadow-2xl relative">
                ...
                <div className="bg-[#0A0603] border border-white/10 rounded-2xl overflow-hidden">
                  <table className="w-full text-left text-xs text-white/80">
                    <thead className="bg-[#241C16] text-[#C8951E] text-[10px] font-mono uppercase">
                      <tr>
                        <th className="p-3">IngrÃ©dient / Actif Botanique</th>
                        <th className="p-3">Dosage PrÃ©cis</th>
                        <th className="p-3">RÃ´le Dermo-Botanique</th>
                      </tr>
                    </thead>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-[#241C16]/50 border border-white/5 p-4 rounded-2xl text-xs">
                ...
                <div>
                  <span className="text-[10px] text-white/40 font-mono block uppercase">PÃ©remption Produit Frais</span>
              ...
              <div className="border-t border-white/10 pt-4 flex items-center justify-between">
                <span className="text-xs text-white/40 font-sans">
                  PrÃ©parÃ© par : <strong className="text-white">{selectedOrder.preparedBy}</strong>
                </span>
                ...
                <div className="flex gap-2">
                  ...
                  {selectedOrder.status === 'ready' && (
                    ...
                    >
                      <CheckCircle2 className="w-4 h-4" />
                      Livrer Ã  la Cliente (Caisse POS)
                    </Button>
                  )}
                </div>
              </div>
        ...
        <div className="bg-[#1A1410] border border-white/10 rounded-3xl p-6 space-y-4 shadow-2xl">
          <div className="flex justify-between items-center">
            <h2 className="font-display font-bold text-lg text-white flex items-center gap-2">
              <Beaker className="text-[#C8951E]" /> Stock des MatiÃ¨res PremiÃ¨res Brutes au Labo
            </h2>
            <span className="text-xs text-white/40 font-mono">Mis Ã  jour en temps rÃ©el</span>
          </div>
              ...
              <div key={idx} className="bg-[#0A0603] border border-white/10 p-4 rounded-2xl space-y-2">
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-[9px] font-bold px-2 py-0.5 rounded-full ${
                    ...
                    {item.status === 'low' ? 'âš ï¸ RÃ©approvisionner' : 'Stock Optimal'}
                  </span>
                </div>
        ...
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 print:p-0 print:bg-white print:static">
          <div className="bg-white text-black p-6 rounded-2xl max-w-sm w-full space-y-3 font-sans border-2 border-black print:border-none shadow-2xl">
            <div className="text-center border-b border-black pb-2">
              <h2 className="font-bold text-lg uppercase tracking-tight">KÃˆNÃˆ COSMÃ‰TIQUE LAB</h2>
            ...
            <div className="space-y-1 text-xs">
              ...
              <p><strong>PÃ‰REMPTION :</strong> {labelModalOrder.expiryDate}</p>
            </div>

            <div className="border-t border-black pt-2 text-[9px] text-gray-700">
              <strong>INGRÃ‰DIENTS :</strong> {labelModalOrder.ingredients.map(i => `${i.name} (${i.amount})`).join(', ')}.
            </div>
